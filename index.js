...
function multiplyArray(array) {
  let plus = 1;    // Ініціалізуємо змінну для зберігання добутку
  for (const n of array) {    // Проходимося по всіх значеннях масиву
    plus *= n;  // Множимо кожен елемент на загальний добуток
  }
  // Повертаємо добуток
  return plus;
}
...
function printUppercase(array) {
  for (const n of array) { // Проходимося по всіх елементах масиву
    console.log(n.toUpperCase());  // Перетворюємо кожен елемент в верхній регістр та виводимо на консоль
  }
...
function findMinMax([firstElement, ...restElements]) {
  ...
  for (const n of restElements) {  
    if (n < min) {    // Перевіряємо, чи поточний елемент менший за мінімальне значення
      // Якщо так, оновлюємо мінімальне значення
      min = n;
    } else if (n > max) {   // Перевіряємо, чи поточний елемент більший за максимальне значення
      max = n;    // Якщо так, оновлюємо максимальне значення
    }
...
function addTenToEach(array) {
  ...
  for (const index in array) {
     ...
     newarray = [...newarray, array[index] + 10];
...
function multiplyByIndex(arr) {
  ...
  for (const index in arr) {
    ...
    newarr2 = [...newarr2, arr[index] * index];
  }
